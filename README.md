...
### ⚖️ Trade-offs Made
*   **STOMP over WebSockets vs. Server-Sent Events (SSE):** We chose WebSockets (via STOMP) because auctions require bidirectional communication (users sending bids instantly, server broadcasting them instantly). While SSE is simpler for one-way updates, WebSockets provide the sub-millisecond latency required for a competitive "live room" environment.
*   **Polling vs. Global WebSockets for Dashboards:** The main auction lists and admin dashboards use a 5-second polling interval rather than a global WebSocket connection. This trade-off significantly reduces the constant server load of maintaining hundreds of global socket connections, reserving heavy WebSocket usage purely for active participants inside a specific "Live Room".
*   **Database Constraints vs. Application Logic Validation:** We validate bid amounts heavily in the application layer (Spring Boot) and UI layer (React) rather than relying purely on complex SQL constraints at the database level. This trade-off means the database is slightly less strict on its own, but allows us to instantly return user-friendly, descriptive error messages to the frontend before touching the database.
*   **JWT vs. Session Cookies:** We chose Stateless JWTs (JSON Web Tokens) over traditional server-side Sessions. While this requires slightly more complex frontend logic (attaching Bearer tokens to headers), it allows our Spring Boot API to remain completely stateless. This trade-off makes horizontal scaling (adding more servers) much easier in the future since no server has to "remember" who is logged in.
*   **ElementCollection for Images vs. Separate Entity Table:** When upgrading to multi-image galleries, we used JPA's `@ElementCollection` to store an array of URLs rather than creating a full `@Entity` for `MotorcycleImage`. This trade-off prevents us from easily adding metadata (like "isPrimary" or "caption") to individual images in the future, but massively reduced the immediate code complexity and eliminated the need for complex cascading saves.